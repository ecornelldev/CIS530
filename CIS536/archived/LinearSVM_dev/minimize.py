...
def minimize_helper(loss, loss_grad, w, b, xTr, yTr, C):
    def loss_lambda(X):
        return loss(X[:-1], X[-1], xTr, yTr, C)
    
    def grad_lambda(X):
        return np.append(*loss_grad(X[:-1], X[-1], xTr, yTr, C))

    X = np.append(w, np.array(b))

    Xfinal, fX, i = minimize(X, loss_lambda, grad_lambda, maxnumlinesearch=1000)
    print(fX)
    print('Stop after ', i)
